...
def C_Delta_T(array1_tiempo,array4_Tsky,n,Var_comp_1mk_random,mcmc_run_number,step,ideal,scatter):
    ...
    if step==100 and scatter==0:
        print('nominal case: paper, scatter=0')
    ...
    elif step==100 and scatter==1:
        print('nominal case: paper, scatter=1')
        t_load_r=8*np.ones(n)
        #t_load_r=np.zeros(n)
        Tw=np.ones(n)*Var_comp_1mk_random.get('Tw')[mcmc_run_number] # T. window 300K
        Tirf=np.ones(n)*Var_comp_1mk_random.get('Tirf')[mcmc_run_number] # T. IR filter 50K
        Txps=np.zeros(n)*Var_comp_1mk_random.get('Txps')[mcmc_run_number]
        Tfhs= np.ones(n)*Var_comp_1mk_random.get('Tfhs')[mcmc_run_number] # T. sky feed-horn 5K
        Tomts=np.ones(n)*Var_comp_1mk_random.get('Tomts')[mcmc_run_number] # T. OMT sky 5K
        ThX=np.ones(n)*Var_comp_1mk_random.get('ThX')[mcmc_run_number]# T. Hybrid X

        Tl=np.ones(n)*Var_comp_1mk_random.get('Tl')[mcmc_run_number] # T. Cold-Load
        Tfhl=np.ones(n)*Var_comp_1mk_random.get('Tfhl')[mcmc_run_number] # T.load feed-horn
        Tomtl=np.ones(n)*Var_comp_1mk_random.get('Tomtl')[mcmc_run_number] # T. OMT load
        ThY=np.ones(n)*Var_comp_1mk_random.get('ThY')[mcmc_run_number] # T.Hybrid Y
        Tenv2_LNA=np.ones(n)*Var_comp_1mk_random.get('Tenv2_LNA')[mcmc_run_number] # T. environment 2
        
        Tenv1=np.ones(n)*Var_comp_1mk_random.get('Tenv1')[mcmc_run_number] # T. environment 1 
        Tenv2=np.ones(n)*Var_comp_1mk_random.get('Tenv2')[mcmc_run_number] # T. environment 2
        Troom=np.ones(n)*Var_comp_1mk_random.get('Troom')[mcmc_run_number] # T. cryos 1 (T.room)

        T_ext_cry=np.ones(n)*Var_comp_1mk_random.get('T_ext_cry')[mcmc_run_number] # 
        T_BEM_filter=np.ones(n)*Var_comp_1mk_random.get('T_BEM_filter')[mcmc_run_number] # 
        T_after_filt_BEM=np.ones(n)*Var_comp_1mk_random.get('T_after_filt_BEM')[mcmc_run_number] # 
        T_after_ampl_DC=np.ones(n)*Var_comp_1mk_random.get('T_after_ampl_DC')[mcmc_run_number] # 
        T_FPGA=np.ones(n)*Var_comp_1mk_random.get('T_FPGA')[mcmc_run_number]#

        Tcryo1=np.ones(n)*Var_comp_1mk_random.get('Tcryo1')[mcmc_run_number] # T. cryos 2 (1st stage)
        Tcryo2=np.ones(n)*Var_comp_1mk_random.get('Tcryo2')[mcmc_run_number] # T. cryos 3  (2nd stage)
        ...
        t_load_r=np.ones(n)*y_ajustada_append[10][step] # T. Cold-Load
    ...
    if ideal==1:
        #print('ideal case')
        t_load_r=8*np.ones(n)
        Tw=np.ones(n)*0 # T. window 300K
        Tirf=np.ones(n)*0 # T. IR filter 50K
        Txps=np.ones(n)*0
        Tfhs= np.ones(n)*0 # T. sky feed-horn 5K
        Tomts=np.ones(n)*0# T. OMT sky 5K
        ThX=np.ones(n)*0# T. Hybrid X

        Tl=np.ones(n)*0 # T. Cold-Load
        Tfhl=np.ones(n)*0 # T.load feed-horn
        Tomtl=np.ones(n)*0 # T. OMT load
        ThY=np.ones(n)*0 # T.Hybrid Y
        
        Tenv2_LNA=np.zeros(n)# T. environment 2
        Tenv1=np.zeros(n) # T. environment 1 
        Tenv2=np.ones(n)*1e-20 # T. environment 2
        Troom=np.zeros(n) # T. cryos 1 (T.room)

        T_ext_cry=np.zeros(n) # 
        T_BEM_filter=np.zeros(n) # 
        T_after_filt_BEM=np.zeros(n) # 
        T_after_ampl_DC=np.zeros(n) # 
        T_FPGA=np.ones(n) #

        Tcryo1=np.zeros(n) # T. cryos 2 (1st stage)
        Tcryo2=np.zeros(n) # T. cryos 3  (2nd stage)
        
        SPOw=np.zeros(n) # SPO window
       
        SPOirf=np.zeros(n)# SPO IR filter
        SPO=np.zeros(n)# SPO Cold-load

        RLirf=np.ones(n) # Return Loss IR filter
        RLlna1s=np.ones(n) # Return Loss lna1
        RLlna2s=np.ones(n) # Return Loss lna2
        RLlna3l=np.ones(n) # Return Loss lna3
        RLlna4l=np.ones(n) # Return Loss lna4
    ...
    # print('mean:L W power',(1-np.mean(R)))
    #R=np.ones(n)*(1-0.986155852581798)
    #Z=np.ones(n)*(1-0.986155852581798)
    ...
    Wl=Q## il FH
    Ws=Q 
    #Wl=np.ones(n)*(1-0.9748399788806699)
    #Ws=np.ones(n)*(1-0.9748399788806699)
    # print('max:L fh',10*np.log10(1-max(Wl)))
    # print('min:L fh',10*np.log10(1-min(Wl)))
    # print('mean:L fh',10*np.log10(1-np.mean(Wl)))
    # print('mean:L fh power',(1-np.mean(Wl)))
    
    Vl=V ## rl FH
    Vs=V 
    #Vl=np.ones(n)*(1-0.9748399788806699)
    #Vs=np.ones(n)*(1-0.9748399788806699)
    
    Xl=X ## RL OMT
    Xs=X
    #Xl=np.ones(n)*(1-0.9977061036224686)
    #Xs=np.ones(n)*(1-0.9977061036224686)
    ...
    #print('GainBEM',np.mean(GainBEM))
    #GainBEM=np.ones(n)*922.9324402891627
    ...
    if ideal==1:
        hw=np.ones(n) # h Window
        hirf=np.ones(n)
        hXPS=np.ones(n)
        hfhs=np.ones(n)

        homts=np.ones(n) # h sky OMT
        hfhl=np.ones(n) # h Load feed-horn

        homtl=np.ones(n) # h load omt
        hhyb1=np.ones(n) # h hybrid X
        hhyb2=np.ones(n) # h hybrid Y
        hlna1=np.ones(n) # h LNA1
        hlna2=np.ones(n) # h LNA2
        hlna3=np.ones(n) # h LNA3
        hlna4=np.ones(n) # h LNA4
        hload=np.ones(n) #h Cold-load
        hlnaBEM=np.ones(n)
        hfilterBEM=np.ones(n) ### 
        hlnaDC=np.ones(n)
        hfilDC=np.ones(n)
        ### effective losses of hybrids and LNAs BEM stage
        Aphi1=((hhyb1*hlna1)/(2*np.ones(n)))
    ...
    if ideal==1:
        a3=hfhl*homtl*0.5*(np.ones(n))*(1) #### beta LOAD
    ### effective SPO 
    ...
    #a2_t=hw*hirf*hXPS*hfhs*homts*0.5  #### beta SKY
    #a3_t=hfhl*homtl*(np.ones(n)-M)*(1-SPO)*0.5 #### beta LOAD
    ...
    if ideal==1:
        hyb_effect1=(ThX*1)+(Tenv2*1) # (T.hybX* Il.HybX)+ (T.env2*RL.hybX)
        hyb_effect2=(ThY*1)+(Tenv2*1) # (T.hybY* Il.HybY)+ (T.env2*RL.hybY)
    ...
    if ideal==1:
        sum_tem_IL_sky=(Tw*1*hirf*hfhs*homts*0.5*(1))+ (Tirf*1*hfhs*homts*0.5*(1))+(Tfhs*1*homts*0.5)+(Tomts*1)
        sum_tem_IL_load=(Tfhl*1*homtl*0.5)+(Tomtl*1)
    ...
    if ideal==1:
        sum_R_sky=(1*hirf*hfhs*homts*0.5*(Tenv1/Tenv2)*(1))+(1*hfhs*homts*0.5*(1))+(1*homts*0.5)+(1)
        sum_R_load=(1*(1)*hfhl*homtl*0.5)+(1*homtl*0.5)+(1)
    ...
    #print('prueba borrar: Gtot',np.mean(Gtot))
    
    T_sky_bet_total=(2*beta_sky2+2*beta_sky4)/Gtot
    T_load_bet_total=(2*beta_load2+2*beta_load4)/Gtot
    
    #print('prueba borrar: T_sky_bet_total',np.mean(T_sky_bet_total))
    #print('prueba borrar: T_load_bet_total',np.mean(T_load_bet_total))
    ...
    #print('prueba borrar: Tn_tot_sky',np.mean(Tn_tot_sky))
    #print('prueba borrar: T_load_bet_total',np.mean(T_load_bet_total))
    ...
    """""
    plt.figure(1)
    plt.plot(xx1, 10*np.log10(Xs), color='k', label='OMT')
    plt.figure(2)
    
    plt.plot(xx1, (Xs), color='k', label='OMT')
    ...
    axs[0].plot(xx1, 10*np.log10(np.ones(n)-Q1), label='Hybrid', color='c',linewidth=5)
    ...
    axs[0].plot(xx1, 10*np.log10(np.ones(n)-IL_irfilter_t), label='IRF', color='tab:orange')
    axs[0].tick_params(labelsize=15)
    axs[0].set_ylabel(r'$\rm S_{21}\ [dB]$', fontsize=20)
    axs[0].legend(loc='best', fontsize=10, ncol=5)
    ...
    axs[1].plot(xx1, 10*np.log10(Xs), color='k', label='OMT')
    ...
    plt.tight_layout()
    plt.savefig('/home/aarriero/Documents/Angela_cmb/four_year/TOD_V_dic2025/B_sky_signal_results/A_relative_condi.pdf')

    #plt.savefig('/home/aarriero/Documents/Angela_cmb/four_year/TOD_V_dic2025/B_results_plots/A_relative_condi.pdf')
    
    plt.figure()
    fig, axs = plt.subplots(2, 1, figsize=(8, 5), sharex=True)  # 2x2 cuadrícula de subplots
    axs = axs.flatten()  # <-- aplanar la matriz de ejes (4 subplots en una lista)
    ####################### Subplot 3 #############################
    axs[0].plot(xx1, 10*np.log10(G1), label=r'$\rm Gain$', color='tab:purple')
    axs[0].tick_params(labelsize=15)
    ...
    ax2 = axs[0].twinx()
    ax2.plot(xx1, U1, label=r'$\rm Noise $', color='tab:purple',linestyle='dashed')
    ax2.set_ylabel('N [K]', fontsize=20, color='k')
    ax2.tick_params(axis='y', labelsize=15, labelcolor='k')
    ax2.set_ylim(0, 10)
    ax2.legend(loc='lower right')
    ax2.set_title('Low Noise Amplifiers', fontsize = 14)
    ...
    axs[1].set_title('BEM and DC amplifiers', fontsize = 14)
    #axs[1].grid()
    
    # Segundo eje Y para el subplot 4
    ax3 = axs[1].twinx()
    ax3.plot(xx1, TnBem_var, label=r'$\rm N_{BEM}$', color='r',linestyle='dashed')
    ax3.plot(xx1, TnDC, label=r'$\rm N_{DC}$', color='b',linestyle='dashed')
    ...
    plt.tight_layout()
    #plt.savefig('/home/aarriero/Documents/Angela_cmb/four_year/TOD_V_dic2025/B_sky_signal_results/A_relative_condi_amplifiers.pdf')
    #plt.savefig('/home/aarriero/Documents/Angela_cmb/four_year/TOD_V_dic2025/B_results_plots/A_relative_condi_amplifiers.pdf')
    """""
    ##########################################################################################################
    ##########################################################################################################
    ##########################################################################################################
    ...
    ##########################################################################################################
        ...
        total_I1_BEM_atmos_cmb.append(sky_gth2/Gtot)
        total_I2_BEM_atmos_cmb.append(load_gth2/Gtot)
    ...
    return total_BEM_atmos_cmb,total_I1_BEM_atmos_cmb,total_I2_BEM_atmos_cmb,freq_spec,Tlss_resta,y_ajustada_append,T_sky_bet_total,T_load_bet_total,extra_loss_hybs

def Weights_subBands(array1_tiempo,freq_spec,total_BEM_atmos_cmb,total_I1_BEM_atmos_cmb,total_I2_BEM_atmos_cmb,T_sky_bet_total,T_load_bet_total, atmosphere_input,Tlss_resta,array5_loaded):
    ...
    band_pass_prom_I1=[]
    band_pass_prom_I2=[]
    band_pass_prom_betasky=[]
    band_pass_prom_betaload=[]
    band_pass_prom_atmosphere_input=[]
    band_pass_prom_Tlss_resta=[]
    band_pass_prom_array5_loaded=[]
    ...
    for i in range(np.size(array1_tiempo)):### tiempo de observacion en segundos
        for j in range(size_arr_freq): ####### frecuencias de 10 a 20 Ghz
            ...
            band_pass_prom_I1.append(np.mean(total_I1_BEM_atmos_cmb[i][indices]))
            band_pass_prom_I2.append(np.mean(total_I2_BEM_atmos_cmb[i][indices]))
            band_pass_prom_atmosphere_input.append(np.mean(atmosphere_input[i][indices])) 
            band_pass_prom_Tlss_resta.append(np.mean(Tlss_resta[i][indices]))
            
            
    for i in range(size_arr_freq):
        x = new_freq[0]+(240e6*i)+(i*1e7) #### elijo la frecuencia principal (10Ghz)+ 240Mhz*(0)+(0)*1e7
        x_f = x+240e6
        
        indices = np.where((new_freq >= x) & (new_freq <= x_f))[0]
        band_pass_prom_betasky.append(np.mean(T_sky_bet_total[indices]))
        band_pass_prom_betaload.append(np.mean(T_load_bet_total[indices]))
        band_pass_prom_array5_loaded.append(np.mean(array5_loaded[indices]))
           

    band_pass_prom_t=np.reshape(band_pass_prom, (np.size(array1_tiempo),size_arr_freq))
    band_pass_prom_t_I1=np.reshape(band_pass_prom_I1, (np.size(array1_tiempo),size_arr_freq))
    band_pass_prom_t_I2=np.reshape(band_pass_prom_I2, (np.size(array1_tiempo),size_arr_freq))
    band_pass_prom_t_atmosphere_input=np.reshape(band_pass_prom_atmosphere_input, (np.size(array1_tiempo),size_arr_freq))
    band_pass_prom_t_Tlss_resta=np.reshape(band_pass_prom_Tlss_resta, (np.size(array1_tiempo),size_arr_freq))
    
    band_pass_prom_t_betasky=np.reshape(band_pass_prom_betasky, (size_arr_freq))
    band_pass_prom_t_betaload=np.reshape(band_pass_prom_betaload, (size_arr_freq))
    band_pass_prom_t_array5_loaded=np.reshape(band_pass_prom_array5_loaded, (size_arr_freq))
    ...
    return sub_band_def, band_pass_prom_t,index_band_pass,band_pass_prom_t_I1,band_pass_prom_t_I2,band_pass_prom_t_betasky,band_pass_prom_t_betaload,band_pass_prom_t_atmosphere_input,band_pass_prom_t_Tlss_resta,band_pass_prom_t_array5_loaded
...
def noise_instrument(array1_tiempo,freq_spec,band_pass_prom_t,band_pass_prom_t_I1,band_pass_prom_t_I2,ideal,noise,f_knee,alpha):
    ###############################################################################################
    ########################## Definitions #################################
    ...
    White_noise_subbands_I1=[]
    White_noise_subbands_I2=[]
    ...
    for i in range(size_arr_freq):
        for j in range(np.size(array1_tiempo)):
            ...
            signoiseI1    = band_pass_prom_t_I1[j][i]/(np.sqrt(250e6*t_sampling))
            sigma_new_I1.append(signoiseI1)
            signoiseI2    = band_pass_prom_t_I2[j][i]/(np.sqrt(250e6*t_sampling))
            sigma_new_I2.append(signoiseI2)
            
    reshape_White_noise_I1=np.reshape(sigma_new_I1, (size_arr_freq,np.size(array1_tiempo) ))
    reshape_White_noise_I2=np.reshape(sigma_new_I2, (size_arr_freq,np.size(array1_tiempo) ))

    sigma_I1=reshape_White_noise_I1
    sigma_I2=reshape_White_noise_I2
    
    White_noise_suma_cuadratica_I1_I2=[]
    White_noise_suma_cuadratica_I1_I2=np.sqrt(np.array(sigma_I1)**2+np.array(sigma_I2)**2)
    print('White_noise_suma_cuadratica_I1_I2',np.shape(White_noise_suma_cuadratica_I1_I2))
    
    #print(reshape_White_noise[0])
    for i in range(size_arr_freq):
        ####################### White_noise ##################################
        media = 0      
        num_muestras = nsamp
        ## White_noise = np.random.normal(loc=media, scale=sigma, size=num_muestras)
        White_noise = np.random.normal(loc=media, scale=np.abs(White_noise_suma_cuadratica_I1_I2[i]), size=num_muestras)
        White_noise_subbands.append(White_noise)
        
        
        
    if ideal==1:
        White_noise_subbands=np.zeros((size_arr_freq))
    ...
    #print('White_noise_subbands',np.mean(White_noise_subbands))
    #print('White_noise_subbands_I1',np.mean(White_noise_subbands_I1))
    #print('White_noise_subbands_I2',np.mean(White_noise_subbands_I2))
    ################################  TOTAL NOISE ####################################################
    ...
    suma_total_signal_whitenoise=np.array(suma_total_signal_whitenoise).T
    
    delete_this=0
    #print('suma_total_signal_whitenoise',np.shape(suma_total_signal_whitenoise))
    #print('suma_total_signal_whitenoise I1',np.shape(suma_total_signal_whitenoise_I1))
    #print('suma_total_signal_whitenoise I2',np.shape(suma_total_signal_whitenoise_I2))
    return sigma_new,White_noise_suma_cuadratica_I1_I2,noise_1f_mean,noise_1f_mean_curve,suma_total_signal_whitenoise,delete_this,White_noise_subbands,White_noise_suma_cuadratica_I1_I2,sigma_I1,sigma_I2
