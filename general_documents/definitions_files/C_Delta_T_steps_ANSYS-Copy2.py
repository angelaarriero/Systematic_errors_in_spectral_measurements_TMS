...
def C_Delta_T(array1_tiempo,array4_Tsky,n,step,ideal):
...
def noise_instrument(array1_tiempo,freq_spec,band_pass_prom_t,band_pass_prom_t_I1,band_pass_prom_t_I2,ideal,noise,f_knee,alpha):
    ...
    reshape_White_noise_I1=np.reshape(sigma_new_I1, (np.size(array1_tiempo), size_arr_freq))
    reshape_White_noise_I2=np.reshape(sigma_new_I2, (np.size(array1_tiempo), size_arr_freq))
    ...
    for i in range(size_arr_freq):
        ...
        White_noise = np.random.normal(loc=media, scale=np.abs(reshape_White_noise[:,i]), size=num_muestras)
        White_noise_subbands.append(White_noise)
        
        White_noiseI1 = np.random.normal(loc=media, scale=reshape_White_noise_I1[:,i], size=num_muestras)
        White_noise_subbands_I1.append(White_noiseI1)
        
        White_noiseI2 = np.random.normal(loc=media, scale=reshape_White_noise_I2[:,i], size=num_muestras)
        White_noise_subbands_I2.append(White_noiseI2)
    ...
    if ideal==1:
        ...
        White_noise_subbands_I1=np.zeros((size_arr_freq))
        White_noise_subbands_I2=np.zeros((size_arr_freq))
    ##################### borrar por prueba
    if noise==1:
        White_noise_subbands=np.zeros((size_arr_freq))
        White_noise_subbands_I1=np.zeros((size_arr_freq))
    ...
    suma_total_signal_whitenoise_I1=[]
    suma_total_signal_whitenoise_I2=[]
    
    for i in range(size_arr_freq):
        a=band_pass_prom_t.T[i]+White_noise_subbands[i]
        suma_total_signal_whitenoise.append(a)
        b=band_pass_prom_t_I1.T[i]+White_noise_subbands_I1[i]
        suma_total_signal_whitenoise_I1.append(b)
        c=band_pass_prom_t_I2.T[i]+White_noise_subbands_I2[i]
        suma_total_signal_whitenoise_I2.append(c)
    ...
    return sigma_new,White_noise_subbands,noise_1f_mean,noise_1f_mean_curve,suma_total_signal_whitenoise,suma_total_signal_whitenoise_I1,suma_total_signal_whitenoise_I2
