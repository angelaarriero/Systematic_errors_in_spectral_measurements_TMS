...
def ajustar_recta(x, y):
    """
    Ajusta una recta y = mx + b a los puntos (x, y).
    Retorna la pendiente (m) y el intercepto (b).
    """
    # El grado 1 indica que queremos una línea recta
    m, b = np.polyfit(x, y, 1)
    return m, b


def Ansys_output_steps(nombre_archivo):
    data = []
    with open(nombre_archivo, 'r') as file: 
        csv_reader = csv.reader(file, quotechar='"')
        for row in csv_reader:
    ...
    Temp_10K=[]
    for i in range(len(data)):
        #component.append(data[i][0])
        Temp_5K.append(data[i][0])
        Temp_6K.append(data[i][1])
        Temp_7K.append(data[i][2])
        Temp_8K.append(data[i][3])
        Temp_9K.append(data[i][4])
        Temp_10K.append(data[i][5])
    
    return Temp_5K,Temp_6K,Temp_7K,Temp_8K,Temp_9K,Temp_10K

def rectas_ANSYS_outputs(nombre_archivo,n):
    Temperature_components_steps=Ansys_output_steps(nombre_archivo)
    Temperature_components_steps_T=np.array(Temperature_components_steps).T
    all_components_rectas=[]
    x= np.array([5, 6, 7, 8, 9, 10])
    x_ajustada=np.linspace(5, 10, num=n)
    y_ajustada_append=[]
    
    for i in range(19):
        pendiente, intercepto =ajustar_recta(x, Temperature_components_steps_T[i])
        y_ajustada = pendiente * x_ajustada + intercepto
        y_ajustada_append.append(y_ajustada)

    return x_ajustada,y_ajustada_append
